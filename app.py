...
def _mask_sensitive(text: str) -> str:
    ...
    text = re.sub(r"(?i)(ssn(?:\s*last\s*4)?\s*(?:is|=|:)?\s*)(\d{4})", r"\1****", text)
    text = re.sub(r"(\d{4}-\d{2}-\d{2})", "****-**-**", text)
    text = re.sub(r"(\b[A-Za-z]+(?:\s+[A-Za-z]+)*\s*,\s*)(\d{4})(\s*,\s*\d{4}-\d{2}-\d{2}\b)", r"\1****\3", text)
    return text

def _save_conversation(history):
    os.makedirs(CONVERSATION_DIR, exist_ok=True)
    file_path = os.path.join(CONVERSATION_DIR, "latest_conversation.json")
    sanitized = [{"role": msg.get("role"), "content": _mask_sensitive(msg.get("content", "")) if isinstance(msg.get("content"), str) else msg.get("content")} for msg in history]
    payload = {"saved_at": datetime.now().strftime("%Y%m%d_%H%M%S"), "messages": sanitized}
    with open(file_path, "w", encoding="utf-8") as f:
        json.dump(payload, f, ensure_ascii=False, indent=2)
